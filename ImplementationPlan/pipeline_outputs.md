...
- **Canonical run layout**: Use `scripts/run_pipeline.py` (no `--legacy`) to write under `outputs/runs/{run_id}/`. Each run has `manifest.json` (config_hash, seed, code_version, artifact index). Legacy flat layout (`outputs/internal/`, etc.) is supported via `--legacy`. See `outputs/README.md`.
