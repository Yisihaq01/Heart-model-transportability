...
The report is generated by `src/evaluation.py` `generate_report()`: Executive Summary and key-finding paragraphs are prose (not JSON); see that function and helpers `_format_executive_summary`, `_key_finding_rq1`–`_key_finding_rq5`.
...
> **Checklist status as of 2026-03-01 (latest full run):**
> Items 1–7, 9, 11 verified against `outputs/runs/` artifacts.
> Item 8 has expected NaNs for Kaggle↔UCI CFS rows (no internal CFS baseline on UCI test sites — documented in report Discussion / Limitations).
> Item 10 not yet enforced (config-hash audit planned).
> Item 12 covered by `reports/compliance/TRIPOD_AI_CHECKLIST.md`.
...
| 8 | No NaN in master results table (except fields that legitimately don't apply) | ☑ (see note) |
